...
/**
 * \brief Get library filename of a specific port entry
 * \param info a #GPPortInfo
 * \param lib a pointer to a char* which will receive the library filename
 *
 * Retrieves the library filename of the passed in GPPortInfo, by reference.
 *
 * \return a gphoto2 error code
 **/
int
gp_port_info_get_library_filename (GPPortInfo info, char **lib) {
	*lib = info->library_filename;
	return GP_OK;
}

/**
 * \brief Set library filename of a specific port entry
 * \param info a #GPPortInfo
 * \param lib a char* pointer which will receive the library filename
 *
 * Sets the library filename of the passed in GPPortInfo
 * This is a libgphoto2_port internal function.
 *
 * \return a gphoto2 error code
 **/
int
gp_port_info_set_library_filename (GPPortInfo info, char *lib) {
	C_MEM (info->library_filename = strdup (lib));
	return GP_OK;
}
